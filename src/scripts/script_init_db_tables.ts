...
const TheDb = new Database(DATABASE_NAME);
...
function init() {
  ...
  try {
    const createChats = TheDb.prepare(`
CREATE TABLE IF NOT EXISTS chats (
  ...
  FOREIGN KEY(thread_id) REFERENCES chat_threads(id));`);
    ...
    const createChatThreads = TheDb.prepare(`
CREATE TABLE IF NOT EXISTS chat_threads (
  id TEXT PRIMARY KEY NOT NULL,
  name TEXT NOT NULL,
  created_timestamp INTEGER NOT NULL);`);

    const createChatsDatas = TheDb.prepare(`
CREATE TABLE IF NOT EXISTS llm_datas (
  id TEXT PRIMARY KEY NOT NULL,
  model TEXT NOT NULL,
  ...
  created_timestamp INTEGER NOT NULL);`);
    ...
    const createMigrations = TheDb.prepare(`
